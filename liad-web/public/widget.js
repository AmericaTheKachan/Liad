(function () {
  ...
  // ─── Markdown renderer ────────────────────────────────────────────────────
  // Lightweight renderer: handles bold, italic, numbered lists, bullet lists,
  // product cards (lines starting with "N. **Name**"), and links.
  function renderMarkdown(text) {
    const container = document.createElement("div");

    // Detect product block pattern: numbered item with bold name + price on next line
    ...
    // Try to render product cards first
    ...
    if (hasProductCards) {
      // Render remaining text after last card
    ...
    // No product cards — render standard markdown
  ...
  function parseInline(text) {
    return text
      ...
      .replace(/\*(.+?)\*/g, "<em>$1</em>")
      // Inline link
      .replace(/\[(.+?)\]\((.+?)\)/g, '<a href="$2" target="_blank" rel="noopener noreferrer" style="color:#3ecf8e;text-decoration:none;">$1</a>');
  }
  ...
  function createWidget() {
    ...
    logoImg.onerror = function () {
      ...
      this.replaceWith(createFallbackLogoSVG("icon-logo"));
    };
    ...
    avatar.id = "liad-panel-header-avatar";
    ...
    sendBtn.innerHTML = `<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M14 8L2 2l2.5 6L2 14l12-6z" fill="#0a0a0a"/></svg>`;
    ...
    return { btn, panel, messages, input, sendBtn };
  }
  ...
  function init() {
    injectStyles();
    const { btn, panel, messages, input, sendBtn } = createWidget();
    const { sendMessage } = initChat(messages);
